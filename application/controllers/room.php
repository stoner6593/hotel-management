...
class Room extends CI_Controller {
	...
	public function add()
	{
		...
		if($this->input->post("room_type") && $this->input->post("min_id") && $this->input->post("max_id"))
		{
			...
			$precio_hora = floatval($this->input->post("precio_hora"));
			$precio_dia = floatval($this->input->post("precio_dia"));

			$rooms_avail = count($this->room_m->getRoomRange($new_room_type, $new_min_id, $new_max_id));

			if($new_min_id>$new_max_id) {
				$viewdata['error'] = "Rango no es válido [$new_min_id, $new_max_id]";
			} else if($rooms_avail!==0) {
				$viewdata['error'] = "Rango no esa disponible [$new_min_id, $new_max_id]";
			} else {
				$this->room_m->addRoomRange($new_room_type, $new_min_id, $new_max_id, $precio_hora, $precio_dia);
				redirect("/room");
			}
		}
	...
	public function edit($room_type, $min_id, $max_id)
	{
		...
		if($this->input->post("room_type") && $this->input->post("min_id") && $this->input->post("max_id"))
		{
			...
			if($new_min_id>$new_max_id) {
				...
				$this->room_m->deleteRoomRange($min_id, $max_id);
				$this->room_m->deleteRoomRangeTypeRental($min_id, $max_id);
				$this->room_m->addRoomRange($new_room_type, $new_min_id, $new_max_id, $precio_hora, $precio_dia);
				redirect("/room");
			}
		}
