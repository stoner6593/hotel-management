...
class Report_m extends CI_Model {
    ...
    function get_customer_freq_list() {
        ...
        $query = $this->db->select("customer.* , SUM(  reservation_price ) as total_paid, COUNT(*) as checkin_count")
                ->from("reservation")->join("customer", "customer.customer_id = reservation.customer_id")
                ->group_by("customer_id")->order_by('checkin_count','DESC')->order_by('total_paid','DESC')->get();
        $data = array();
        foreach ($query->result() as $res) {
            $data[] = $res;
...
/*        $query = $this->db->select("customer.* , SUM(  `room_sales_price` +  `total_service_price` ) as total_paid")
        ...
        /*$query = $this->db->query(
            "SELECT * , COUNT(*) as checkin_count,  SUM(  `room_sales_price` +  `total_service_price` ) 
                    AS total_paid
            FROM room_sales
            JOIN (
        ...
        $query = $this->db->query(
            "select b.*, sum(a.reservation_price) as total_paid,
                count(a.customer_id) as checkin_count
                from reservation a
                inner join customer b on b.customer_id = a.customer_id
                where a.status = 1
                group by a.customer_id limit 5"// HAVING total_paid = max_paid
        );
